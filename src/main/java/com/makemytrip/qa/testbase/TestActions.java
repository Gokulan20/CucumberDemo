...
public class TestActions extends TestBase {
	 ...
	 * 
	 * List<WebElement> priceList = driver.findElements(elementLocator); int
	 * listlength = priceList.size(); int convertedList, finalOutput = 0;
	 * List<Integer> price = new ArrayList<Integer>();
	 * 
	 * for (int i = 0; i < listlength; i++) {
	 * 
	 * String temp = priceList.get(i).getText(); temp = temp.replaceAll("[^\\d]",
	 * ""); convertedList = Integer.parseInt(temp); price.add(convertedList); } if
	 * (value.equalsIgnoreCase("min")) {
	 * 
	 * if (price == null || price.size() == 0) { return Integer.MIN_VALUE; }
	 * List<Integer> sortedlist = new ArrayList<>(price);
	 * Collections.sort(sortedlist);
	 * 
	 * finalOutput = sortedlist.get(0); } else if (value.equalsIgnoreCase("max")) {
	 * 
	 * if (price == null || price.size() == 0) { return Integer.MAX_VALUE; }
	 * List<Integer> sortedlist = new ArrayList<>(price);
	 * Collections.sort(sortedlist);
	 * 
